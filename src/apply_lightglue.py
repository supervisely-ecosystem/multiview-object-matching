...
def apply_lightglue_bounding_box(image_paths: List[str], reference_bbox: sly.Rectangle, device: str = "cpu"):
    ...
    for img_path in image_paths:
        ...
        if ref_features["keypoints"].numel() == 0 or ref_features["descriptors"].numel() == 0:
        ...
        if ref_features["keypoints"].dim() == 2:
            ref_features["keypoints"] = ref_features["keypoints"].unsqueeze(0)
            ref_features["descriptors"] = ref_features["descriptors"].unsqueeze(0)
